...
Vue.config.ignoredElements = ['cht', 'eng']
    ...
    change_B: function(){
      ...
      Swal.fire({
        ...
        async save_item(item)
        {
            ...
            ret = await this.save_measure(item);

            if(ret != "error")
            {
                item.is_checked = 0;
                item.encode = item.encode_edit;
                item.record_cust = JSON.parse(JSON.stringify(item.record_cust_edit));
            }
        },

        async save_measure(item)
        {
            let _this = this;
            var token = localStorage.getItem("token");
            var form_Data = new FormData();
            ...
            form_Data.append("encode", item.encode);
            form_Data.append("encode_edit", item.encode_edit);
            form_Data.append("record_cust", JSON.stringify(item.record_cust));
            form_Data.append("record_cust_edit", JSON.stringify(item.record_cust_edit));
            ...
            try {
                let res = await axios({
                  method: 'post',
                  url: 'api/edit_soldto_dr_save.php',
                  data: form_Data,
                  headers: {
                    "Content-Type": "multipart/form-data",
                  },
