...
$sql = "SELECT staff username FROM staff_list_sea where status = 0 and punch = 1 order by staff";
...
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
    if (isset($row)){
        ...
        $mdate = date("Y/m/d");
        //$mdate = "2020/05/11";
        $subquery = " SELECT id, username, duty_date, duty_time, location, duty_type FROM on_duty_v2 WHERE  duty_date = '" . $mdate . "' AND duty_type in('A', 'B') and username = '" . $row['username']. "' ORDER BY created_at ";
