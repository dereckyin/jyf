...
include_once 'objects/add_or_edit_gcash_expense_recorder_sea_2.php';
...
$priceRecord = new GCashExpenseRecordSea2($db);
//$le = new Leave($db);
...
else
{
    if($action == 1){
        //select all
        try{
            $query = "SELECT * from gcash_expense_recorder_sea_2 where 1 = 1 order by paid_date ";
        ...
        try{
            $query = "SELECT * from gcash_expense_recorder_sea_2 where 1 = 1 ";
            if($start_date!='') {
        ...
        //select by id
        try{
            $query = "SELECT * from gcash_expense_recorder_sea_2 where id = ".$id;
            $stmt = $db->prepare( $query );
            $stmt->execute();
            while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $merged_results[] = $row;
