  ...
  try {
      ...
      switch ($method) {
          case 'GET':
            ...
            $sql = "SELECT 0 as is_checked, id, username, email, status, phili, status_1, status_2, taiwan_read, phili_read, report1, report2, airship, airship_read, sea_expense, sea_expense_v2, gcash_expense_sea, gcash_expense_sea_2, sea_feliix, parts_feliix, is_admin, (SELECT login_time FROM login_history WHERE login_history.uid = user.id ORDER BY login_time desc LIMIT 1) login_time  FROM user where status <> -1 ".($id ? " and id=$id" : '');
        ...
        case 'POST':
            ...
            $sea_feliix = stripslashes(isset($_POST['sea_feliix']) ?  $_POST['sea_feliix'] : 0 );
            $parts_feliix = stripslashes(isset($_POST['parts_feliix']) ?  $_POST['parts_feliix'] : 0 );
            ...
            /* Bind parameters. Types: s = string, i = integer, d = double,  b = blob */
                ...
                $user->sea_feliix = $sea_feliix;
                $user->parts_feliix = $parts_feliix;
