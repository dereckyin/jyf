...
function GetMeasurePersonRecord($id, $db){
    $query = "SELECT distinct coalesce(cp.customer, '') cust, 'contactor_ph' kind, cp.id cust_id
            ...
            SELECT distinct coalesce(cp.customer, '') cust, 'measure_detail' kind, cp.id cust_id
                FROM measure_record_detail rd
                    left JOIN receive_record rc ON
                    rd.record_id = rc.id
