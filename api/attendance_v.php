...
$sql = "SELECT id, staff username FROM staff_list where status <> 'D' and punch = 1 order by staff";
...
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
    if (isset($row)){
        ...
        $mdate = date("Y/m/d");
        //$mdate = "2020/05/11";
        $subquery = " SELECT id, username, duty_date, duty_time, location, duty_type FROM on_duty_v WHERE  duty_date = '" . $mdate . "' AND duty_type in('A', 'B') and username = '" . $row['username']. "' ORDER BY created_at ";
        ...
        $subquery2 = " SELECT id, username, duty_date, duty_time, location FROM  on_duty_v  WHERE  duty_date = '" . $mdate . "' AND duty_type = 'B' and username = '" . $row['username']. "' ORDER BY created_at ";
