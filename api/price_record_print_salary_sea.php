...
if($jwt){ 
    ...
    try {
        ...
        // response in json format
            ...
            $sql = "SELECT account, created_at, category, sub_category, project_name, related_account, details, pic_url, payee, paid_date, cash_in, cash_out, remarks from price_record_salary_sea where is_enabled = true";
            $sql1 = "";
            $sql2 = "";
            $sql3 = "";
            ...
            $sheet->setCellValue('J1', 'Files');
            $sheet->setCellValue('E1', 'Payee');
            $sheet->setCellValue('F1', 'Paid/Received Date');
            $sheet->setCellValue('G1', 'Cash in');
            $sheet->setCellValue('H1', 'Cash out');
            ...
            foreach($merged_results as $row)
            {
                ...
                $sheet->setCellValue('E' . $i, $row['payee']);
                $sheet->setCellValue('F' . $i, $row['paid_date']);
                $sheet->setCellValue('G' . $i, $row['cash_in']);
                $sheet->setCellValue('H' . $i, $row['cash_out']);
                $sheet->setCellValue('I' . $i, $row['remarks']);
