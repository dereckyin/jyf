...
if($jwt){
    ...
    try {
        ...
        // response in json format
            ...
            if($taiwan_pay == 1)
                $recode = $receive_record->TaiwanPayQuery($date_start, $date_end, $container_number);
            else
                $recode = $receive_record->CourierPayQuery($date_start, $date_end, $container_number);
            // response in json format
            $styleArray = array(
                'borders' => array(
                    'allBorders' => array(
            ...
            {
                $sheet->setCellValue('A1', 'Customer Name(Tel #)');
                $sheet->setCellValue('B1', 'Date');
                $sheet->setCellValue('C1', 'NTD');
                $sheet->setCellValue('D1', 'Supplier');

                $i = 2;
                foreach($recode as $row)
                {
                    $sheet->setCellValue('A' . $i, $row['customer']);
                    $sheet->setCellValue('B' . $i, $row['date_receive']);
                    $sheet->setCellValue('C' . $i, $row['courier_money']);
                    $sheet->setCellValue('D' . $i, $row['supplier']);
