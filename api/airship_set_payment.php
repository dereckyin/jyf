...
if($print != "")
    $file_name = 'Airship_Payment_Receipt' . ($exp_dr !== '' ? '_' . $exp_dr : '') . '_' . uniqid() . '.docx';

if(IsExist($id, $conn))
{
    $query = "update airship_records_export set 
        exp_dr = ?, 
        assist_by = ?,
        exp_date = ?, 
        exp_sold_to = ?, 
...
{
    $query = "insert into airship_records_export set 
    measure_detail_id = ?,
    exp_dr = ?, 
    assist_by = ?,
    exp_date = ?, 
...
{
    $is_exist = false;
    $sql = "select id from airship_records_export where measure_detail_id = " . $id;
