...
if (!isset($jwt)) {
    ...
    try {
        $sql = "insert into car_calendar_check (sid, date_use, car_use, driver, time_out, time_in, kind, created_by, created_at, status, feliix) 
        values (:sid, :date_use, :car_use, :driver, :time_out, :time_in, :kind, :created_by, now(), 0, 1)";
        ...
        // update work_calendar_main status
        $sql = "update work_calendar_main set status = :status where id = :id";
