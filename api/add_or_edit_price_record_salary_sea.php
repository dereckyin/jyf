...
include_once 'objects/add_or_edit_price_record_salary_sea.php';
...
$priceRecord = new PriceRecordSalarySea($db);
//$le = new Leave($db);
...
else
{
    if($action == 1){
        //select all
        try{
            $query = "SELECT * from price_record_salary_sea where 1 = 1";
        ...
        //get members
        try{
            $query = "SELECT id, staff username from staff_list_sea where status <> 'D' order by staff";
            $stmt = $db->prepare( $query );
            $stmt->execute();
        ...
        //select by id
        try{
            $query = "SELECT * from price_record_salary_sea where id = ".$id;
            $stmt = $db->prepare( $query );
            $stmt->execute();
            while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $merged_results[] = $row;
