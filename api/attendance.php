...
$sql = "SELECT * FROM duty_user order by username";
...
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
    if (isset($row)){
        ...
        $mdate = date("Y/m/d");
        //$mdate = "2020/05/11";
        $subquery = " SELECT id, username, duty_date, duty_time, location, duty_type FROM on_duty WHERE  duty_date = '" . $mdate . "' AND on_duty.duty_type in('A', 'B') and on_duty.username = '" . $row['username']. "' ORDER BY on_duty.created_at ";
        ...
        $subquery2 = " SELECT id, username, duty_date, duty_time, location FROM  on_duty  WHERE  duty_date = '" . $mdate . "' AND on_duty.duty_type = 'B' and on_duty.username = '" . $row['username']. "' ORDER BY on_duty.created_at ";
