...
include_once 'objects/add_or_edit_gcash_expense_recorder_sea.php';
...
$priceRecord = new GCashExpenseRecordSea($db);
//$le = new Leave($db);
...
else
{
    if($action == 1){
        //select all
        try{
            $query = "SELECT * from gcash_expense_recorder_sea where 1 = 1 order by paid_date ";
        ...
        //select by id
        try{
            $query = "SELECT * from gcash_expense_recorder_sea where id = ".$id;
            $stmt = $db->prepare( $query );
            $stmt->execute();
            while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $merged_results[] = $row;
