...
switch ($method) {
    case 'GET':
        ...
        if ($loading == "1") {
            ...
            $sql = "(SELECT 0 as is_checked, lo.id, shipping_mark, estimate_weight, actual_weight, container_number, seal, so, ship_company, ship_boat, neck_cabinet, shipper, lo.date_sent, lo.etd_date, lo.ob_date, lo.eta_date, lo.date_arrive, COALESCE(lo.date_arrive, '') date_arrive_his, COALESCE(ld.date_sent, '') date_send_his, COALESCE(ld.etd_date, '') etd_date_his, COALESCE(ld.ob_date, '') ob_date_his, COALESCE(ld.eta_date, '') eta_date_his, COALESCE(ld.date_arrive, '') date_arrive_his, broker, remark, (SELECT date_arrive FROM measure WHERE measure.id = measure_num) date_arrive_old, crt_time, crt_user, '9999/99/99' ords  FROM loading lo LEFT JOIN loading_date_history ld ON lo.id = ld.loading_id where  lo.STATUS = '' " . ($id ? " and lo.id=$id" : '') . " AND lo.date_sent = '' ORDER BY lo.date_sent ) ";
            $sql = $sql . "UNION ";
                        ...
                        if($result1 != null)
            {
                while($row = mysqli_fetch_assoc($result1))
                {
                    ...
                    $merged_results[] = array(
                        ...
                        "email_customer" => str_replace (array("\r\n", "\n", "\r", "<P/E>"), ' ', $email_customer),
                        "email" => $email,
