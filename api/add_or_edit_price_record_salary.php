...
include_once 'objects/add_or_edit_price_record_salary.php';
...
$priceRecord = new PriceRecordSalary($db);
//$le = new Leave($db);
...
else
{
    if($action == 1){
        //select all
        try{
            $query = "SELECT * from price_record_salary where 1 = 1";
        ...
        //get members
        try{
            $query = "SELECT id, staff username from staff_list where status <> 'D' order by staff";
            $stmt = $db->prepare( $query );
            $stmt->execute();
        ...
        //select by id
        try{
            $query = "SELECT * from price_record_salary where id = ".$id;
            $stmt = $db->prepare( $query );
            $stmt->execute();
            while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $merged_results[] = $row;
