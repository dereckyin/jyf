...
$sql = "SELECT 0 as is_checked, id, shipping_mark, customer, c_phone, c_fax, c_email, supplier, s_phone, s_fax, s_email, company_title, vat_number, address, crt_time, crt_user  FROM contactor where status = '' " .($id ? " and id in ($id)" : '') ;
...
$sheet->setCellValue('A1', '麥頭');
$sheet->setCellValue('B1', '收件人');
$sheet->setCellValue('C1', '電話');
$sheet->setCellValue('D1', 'E-mail');
$sheet->setCellValue('E1', '寄件人');
$sheet->setCellValue('F1', '電話');
$sheet->setCellValue('G1', 'E-mail');
$sheet->setCellValue('H1', '抬頭');
$sheet->setCellValue('I1', '統編');
...
while ($row = mysqli_fetch_array($result))
{
    ...
    $sheet->setCellValue('D' . $i, $row[6]);
    $sheet->setCellValue('E' . $i, $row[7]);
    $sheet->setCellValue('F' . $i, $row[8]);
    $sheet->setCellValue('G' . $i, $row[10]);
    $sheet->setCellValue('H' . $i, $row[11]);
    $sheet->setCellValue('I' . $i, $row[12]);
    $sheet->setCellValue('J' . $i, $row[13]);
